...
int main(int argc, const char * argv[]) {
    ...
    {
        if (muMin!=muMax) mu[iMu] = muMin + iMu*(muMax-muMin)/(nMu-1);
        ...
        for(int jj=0; jj<nK; jj++) {
            ...
            for(int kk=0; kk<nKz; kk++) {
                ...
                for(int iMu=0; iMu<nMu; iMu++){
                    double Ak_0 = -(1./M_PI)*cimag(1.0/ (I*Gamma - (ep_k - mu[iMu]) ));
                    dos[iMu]       += Ak_0;
                    density_0[iMu] += 1.0/(1.0+exp(1000*(ep_k - mu[iMu]) ));
                ...
                int nn=0; for(nn=0; nn<nT; nn++)
                {
                    for(int iMu=0; iMu<nMu; iMu++)  density[iMu][nn]    += 1.0/(1.0+exp(beta[nn] * (ep_k - mu[iMu]) ));
                    ...
                    int n=0; for(n=0; n<nOmega; n++)
                    {
                        ...
                        for(int iMu=0; iMu<nMu; iMu++) {
                          double A_k = -(1./M_PI)*cimag(1.0/ (z-(ep_k - mu[iMu]) ) );
                          Cv[iMu][nn]       += omega[nn][n]*dfermiDirac_dT[nn][n]*A_k;
                          ...
                          double betaOmega2 = betaOmega*betaOmega;
    ...
    for(iMu=0; iMu<nMu; iMu++)
    {
        ...
        printf("% 4.8f % 4.8f % 4.8f ", mu[iMu], 1.0-f_0*density_0[iMu], f_0*dos[iMu]);
        ...
        nn=0; for(nn=0; nn<nT; nn++)
        {
            ...
            fprintf(fileOut,"%e\t%e\t%e\t%e\t%e\t", mu[iMu], f_0*density_0[iMu], 1.0-f_0*density_0[iMu], f_0*dos[iMu], etaNorm);
